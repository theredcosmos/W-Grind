...
const HeroSection = ({ progress, completed, total, streak, levelData }) => {
  ...
  return (
    <motion.div 
      ...
      className="glass-card relative overflow-hidden flex flex-col h-full group p-8"
    >
      ...
      <div className="z-10 flex-1 flex flex-col justify-between">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-glass-muted text-sm font-semibold">{totalXP} Total XP</span>
          </div>
          
          <h1 className="text-4xl md:text-5xl font-black tracking-tight mb-2 text-transparent bg-clip-text bg-gradient-to-br from-glass-text to-glass-muted">
            Algorithm<br/>Mastery
          </h1>
          <p className="text-glass-muted font-medium text-lg max-w-sm mb-6">Track your journey through algorithmic patterns.</p>
        </div>
        ...
        <div className="space-y-6">
          <div className="space-y-3">
            ...
            <div className="h-3 w-full bg-glass-border/30 rounded-full overflow-hidden backdrop-blur-sm p-[2px]">
              <motion.div 
                initial={{ width: 0 }}
          ...
          <div className="flex items-center gap-3 pt-4 border-t border-glass-border/50">
            <div className="bg-orange-500/10 p-2 rounded-xl border border-orange-500/20 backdrop-blur-sm">
              <Flame size={24} className="text-orange-500" />
            </div>
            <div>
              <div className="text-2xl font-black text-glass-text">{streak} Day{streak !== 1 ? 's' : ''}</div>
              <div className="text-xs font-bold text-glass-muted uppercase tracking-wider">Current Streak</div>
            </div>
          </div>
        </div>
      </div>
